...
def plot1_bar(qoess_with,  figname):
    ...
    unit = bar_width
    webrtc_pos = np.arange(3) * bar_width * 6
    ...
    #plt.rcParams['hatch.color'] = webrtc_color
    #webrtc_qoes = np.array([0.41, 0.38, 0.39])
    #dash_qoes = np.array([0.49, 0.43, 0.25])
    baloss_qoes = np.array([0.45,0.51,0.43])
    oos_qoes = np.array([0.6,0.71,0.61])
    print((oos_qoes - baloss_qoes) / oos_qoes)
    rects_webrtcd = ax.bar(webrtcd_pos, [0.39, 0.34, 0.34], label="WebRTC w/o",
                          color='white', edgecolor=webrtcd_color,
                          yerr=qoe_stds[1], ecolor='black', capsize=10,
                          hatch='//',
                          align='center', alpha=0.9, width=bar_width)
    rects_webrtcdx = ax.bar(webrtcd_pos, [0.39, 0.34, 0.34],
                           color='none', edgecolor='black', linewidth=lwidth,
                           # yerr=qoe_stds[1], ecolor='black', capsize=10,
                           # hatch='//',
                           align='center', width=bar_width)
    rects_webrtc = ax.bar(webrtc_pos, [0.41,0.38,0.39], label="WebRTC",
                          color='white',edgecolor=webrtc_color,
                          yerr=qoe_stds[1], ecolor='black', capsize=10,
                          hatch='//',
                          align='center', alpha=0.9, width=bar_width)
    rects_webrtcx = ax.bar(webrtc_pos, [0.41, 0.38, 0.39],
                          color='none', edgecolor='black',linewidth=lwidth,
                          #yerr=qoe_stds[1], ecolor='black', capsize=10,
                          #hatch='//',
                          align='center',  width=bar_width)
    #plt.rcParams['hatch.color'] = dash_color
    rects_dash = ax.bar(dash_pos, [0.38,0.43,0.25], label="DASH",
                        color='white',edgecolor=dash_color,
                        yerr=qoe_stds[2], ecolor='black', capsize=10,
                        hatch='\\',
                        align='center', alpha=0.9, width=bar_width)
    rects_dashx = ax.bar(dash_pos, [0.38, 0.43, 0.25],
                        color='none', edgecolor='black',linewidth=lwidth,
                        #yerr=qoe_stds[2], ecolor='black', capsize=10,
                        #hatch='\\',
                        align='center',  width=bar_width)

    rects_baloss = ax.bar(baloss_pos, [0.45,0.51,0.43], label='BaLoss',
                          color='white',
                          yerr=qoe_stds[0], ecolor='black', capsize=10,
                          edgecolor=baloss_color,
                          hatch='//',
                          align='center', alpha=0.9, width=bar_width)
    rects_balossx = ax.bar(baloss_pos, [0.45,0.51,0.43],
                         color='none', edgecolor='black', linewidth=lwidth,
                         #yerr=qoe_stds[3], ecolor='black', capsize=10,
                         #hatch='\\',
                         align='center', width=bar_width)
    #rects_advanced_baloss = ax.bar(advanced_baloss_pos, 0.71, label='Advanced BaLoss', color='black',
    #                               #yerr=qoe_stds[0], ecolor='black', capsize=10,
    #                               align='center', alpha=0.9, width=bar_width)
    rects_offline_optimal = ax.bar(offline_pos, [0.6,0.71,0.61], label='OOS',
                                   color='white',
                                   yerr=qoe_stds[0], ecolor='black', capsize=10,
                                   edgecolor=oos_color,
                                   hatch='+',
                                   align='center', alpha=0.9, width=bar_width)
    rects_offline_optimal = ax.bar(offline_pos, [0.6,0.71, 0.61],
                           color='none', edgecolor='black', linewidth=lwidth,
                           # yerr=qoe_stds[3], ecolor='black', capsize=10,
                           # hatch='\\',
                           align='center', width=bar_width)
