...
def plot_twin(bitrates_a,bitrates_pensieve,bitrates_mpc,figname):
    ...
    ax1.legend(lns, labs, bbox_to_anchor=(0.75, 0.4) , loc='upper right',
                  fontsize=asize, columnspacing=0.5, ncol=2)
    #ax.grid()


    ax1.set_yticks([1.5,6,10,14])
    ax1.set_yticklabels([1.5,6,10,14])
...
def plot_twin_loss(loss_a,loss_b,figname):
    # setup the background template of plt
    asize = 28
    #bsize = 35
    lwidth = 5
    ...
    lns1a = ax2.plot(loss_a,color="red",
             ...
             label="Pensieve",
             linewidth=lwidth)
    lns1b = ax2.plot(loss_b, color="blue",
                    label="Pensieve+LossAdaptation",
                    linewidth=lwidth, linestyle='-')
    #lns1c = ax.plot(bitrates_mpc, color=bitrate_color_c,
    #                label="Robust-MPC",
    #                linewidth=lwidth, linestyle='-')
...
def plot_twin_qoe(qoe_a,qoe_b,figname):
    # setup the background template of plt
    asize = 28
    #bsize = 35
    lwidth = 5
    ...
    ax3.set_ylabel("QoE", fontsize=yfontsize, color=bitrate_color_p)

    #draw the trace1 on fig1's canvas
    lns1a = ax3.plot(qoe_a,color="red",
             linestyle='-',
             label="Pensive",
             linewidth=lwidth)
    lns1b = ax3.plot(qoe_b, color="blue",
                    label="Pensieve+LossAdaptation",
                    linewidth=lwidth, linestyle='-')
    #lns1c = ax.plot(bitrates_mpc, color=bitrate_color_c,
    #                label="Robust-MPC",
    ...
    lns = lns1a + lns1b
    labs = [l.get_label() for l in lns]
    ax3.legend(lns, labs, bbox_to_anchor=(0.75, 0.5) , loc='upper right',
                  fontsize=asize, columnspacing=0.5, ncol=2)
    #ax.grid()
...
if __name__=="__main__":
    ...
    loss_trend = np.arange(0, 0.0051, 0.0001)
    loss_a[:51] += loss_trend
    ...
    qoe_b = compute_qoe(bitrates_pensieve,loss_b)
    print(qoe_a)
    #bitrates_b = gen_trace_bitrate_b(trace_len)
    plot_twin(bitrates_a,bitrates_pensieve,bitrates_mpc, "qoe_understand1_bandwidth")
    plot_twin_loss(loss_a*5000, loss_b * 5000, "qoe_understand1_loss")
    plot_twin_qoe(qoe_a, qoe_b, "qoe_understand1_qoe")